...
class MainGame():
    ...
    def playBackgroundMusic(self):
        # 创建音乐类的对象
        m = Music(r'D:\python\pythonProgram\tankWar\tankMaterials\img\start.wav')
        m.playMusic(0)
    # v1.14 新增创建墙壁的函数
    def creatSteelWall(self):
        for i in range(10):
            # the size of steelWall is 60*60
            wall = steelWall(60*i,280)
            MainGame.wall_list.append(wall)

    # v1.14 新增展示墙壁的方法
    def displayWall(self):
        for wall in MainGame.wall_list:
            wall.displaySteelWall()
    ...
    def displayP1Tank(self):
        ...
        if MainGame.P1_TANK and MainGame.P1_TANK.live:
            ...
            if not MainGame.P1_TANK.stop:
                ...
                MainGame.P1_TANK.collideBetweenTankAndSteelWall()
        else:
            # del 表示删除
    ...
    def displayEnermyTank(self):
        for eTank in MainGame.enermy_tank_list:
            if eTank.live:
                ...
                eTank.displayEnermyTank()
                # v1.16 调用坦克碰撞墙壁的方法
                eTank.collideBetweenTankAndSteelWall()
            else:
    ...
    def displayP1TankBullet(self):
        for bullet in MainGame.bullet_list:
            ...
            if bullet.live:
                ...
                # v1.15调用子弹碰撞墙壁的方法
                bullet.collideBetweenBulletAndSteelWall()
            else:
    ...
    def displayEnermyTankBullet(self):
        ...
        for eBullet in MainGame.enermy_bullet_list:
            ...
            if eBullet.live:
                ...
                # v1.15调用子弹碰撞墙壁的方法
                eBullet.collideBetweenBulletAndSteelWall()
            else:
...
class Tank(BaseItem):
    ...
    # v1.16 新增坦克与墙壁碰撞的方法
    def collideBetweenTankAndSteelWall(self):
        for steelWall in MainGame.wall_list:
            result = pygame.sprite.collide_rect(steelWall,self)
            if result:
                # 碰撞之后坐标调用坐标还原方法
                self.stay()
    def fire(self):
...
class Bullet(BaseItem):
    ...
    # v1.15 新增子弹与墙壁的碰撞
    def collideBetweenBulletAndSteelWall(self):
        for steelWall in MainGame.wall_list:
            result = pygame.sprite.collide_rect(steelWall,self)
            if result:
                self.live = False
...
class Explode(BaseItem):
    ...
    def __init__(self,rect):
        self.images =[
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast0.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast1.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast2.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast3.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast4.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast5.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast6.gif'),
            pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\blast7.gif')
        ]
        self.rect = rect
...
# v1.14 新增墙壁类
class steelWall(BaseItem):
    #墙壁需要分类处理
    def __init__(self,left,top):
        self.image = pygame.image.load(r'D:\python\pythonProgram\tankWar\tankMaterials\img\bigSteelWall.gif')
        self.rect = self.image.get_rect()
        self.rect.left = left
        self.rect.top = top

    def displaySteelWall(self):
        MainGame.window.blit(self.image,self.rect)
...
class Music():
    ...
    def playMusic(self,count):
        # background music loop pla, so we use parameter -1
